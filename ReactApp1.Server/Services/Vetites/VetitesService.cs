...
namespace ReactApp1.Server.Services.Vetites
{
    public class VetitesService(DataBaseContext context, IConfiguration configuration, IEmailService emailService) : IVetitesService
    {
        ...
        {
            var vetitesek = await context.Vetites.IgnoreAutoIncludes().Include(x=>x.Film).IgnoreAutoIncludes().Include(x => x.Terem).IgnoreAutoIncludes().Include(x => x.Film).ThenInclude(x => x.Images).IgnoreAutoIncludes().ToListAsync();
        ...
        {
            var vetites = await context.Vetites.IgnoreAutoIncludes().Include(x => x.Film).IgnoreAutoIncludes().Include(x=>x.VetitesSzekek).IgnoreAutoIncludes().Include(x => x.Terem).IgnoreAutoIncludes().Include(x => x.Film).ThenInclude(x => x.Images).IgnoreAutoIncludes().ToAsyncEnumerable().WhereAwait(async x => await ValueTask.FromResult(x.id == id)).ToListAsync();
